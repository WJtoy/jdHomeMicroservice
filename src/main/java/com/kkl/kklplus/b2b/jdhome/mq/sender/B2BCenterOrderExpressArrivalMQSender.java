package com.kkl.kklplus.b2b.jdhome.mq.sender;
    ...
    public void send(MQB2BCenterOrderExpressArrivalMessage.B2BCenterOrderExpressArrivalMessage message) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                log.error("工单到货通知队列发送失败:{}, {}", throwable.getLocalizedMessage(), msg);
                String msgJson = new JsonFormat().printToString(msg);
                sysLogService.insert(1L,msgJson,"工单到货通知队列发送失败：" + throwable.getLocalizedMessage(),
                        "工单到货通知队列发送失败", B2BMQConstant.MQ_B2BCENTER_ORDER_EXPRESS_ARRIVAL, "");

                return null;
            });
        } catch (Exception e) {
            String msgJson = new JsonFormat().printToString(message);
            log.error("工单到货通知队列发送失败:{}, {}", e.getMessage(), msgJson);
            sysLogService.insert(1L,msgJson,"工单到货通知队列发送失败：" + e.getMessage(),
                    "工单到货通知队列发送失败", B2BMQConstant.MQ_B2BCENTER_ORDER_EXPRESS_ARRIVAL, "");
        }
    }
