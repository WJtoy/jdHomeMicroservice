package com.kkl.kklplus.b2b.jdhome.mq.receiver;
...
public class B2BCenterOrderMQReceiver {
    ...
    //@RabbitListener(queues = B2BMQConstant.MQ_B2BCENTER_ORDER_EXPRESS_ARRIVAL)
    public void onMessage2(Message message, Channel channel) throws Exception {
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        try {
            MQB2BCenterOrderExpressArrivalMessage.B2BCenterOrderExpressArrivalMessage b2BOrderMessage =
                    MQB2BCenterOrderExpressArrivalMessage.B2BCenterOrderExpressArrivalMessage.parseFrom(message.getBody());
            log.error("{}", b2BOrderMessage.toString());
        }
        catch (Exception e) {
            e.printStackTrace();
